...
from flask import Flask, request, flash, url_for, jsonify, Response, send_from_directory
...
def run():
    task = ga.apply_async(
        args=[request.json['filename'], request.json['generation']])
...
def taskstatus(task_id):
    ...
    #         'target_generatiion': task.info.get('target_generatiion', 1),
    #         'current_fitness': task.info.get('current_fitness', 0),
    #         'fitness_val_of_best_sol': task.info.get('fitness_val_of_best_sol', 0),
    #         'index_of_best_sol': task.info.get('index_of_best_sol', 0),
    #         'best_fitness_val_reached_in_gen': task.info.get('best_fitness_val_reached_in_gen', 0),
    #         'status': task.info.get('status', '')
    #     }
    else:
        response = {
